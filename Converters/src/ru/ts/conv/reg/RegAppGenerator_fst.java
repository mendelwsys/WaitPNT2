 ...
 */
public class RegAppGenerator_fst extends BaseInitAble
...
//	}
	...
	protected void setRuleDesc(IXMLObjectDesc ruledesc, String typeId) throws Exception
	{
...
//		final InitAbleMapTransformer trans = (InitAbleMapTransformer) bcontext.getBuilderByTagName(transdesc.getTagname()).initByDescriptor(transdesc);
...
//		trans.reinitMapTransformer(dstpar);
		...
		for (String typeid : type2record.keySet())
		{
			...
			MemEditableStorageLr memstor = new MemEditableStorageLr(pdir, groupname);
			...
			for (RegReader.Record gobj : objs)
			{
				...
				final double[] xdim = gobj.getDimARRAY(RegReader.Record.XDIM);
				final double[] ydim = gobj.getDimARRAY(RegReader.Record.YDIM);
...
//				}
				...
				{
					DefAttrImpl attr = new DefAttrImpl("TNAME", "non");
					objatrrs.put(attr.getName(), attr);
				}
...
//					ruledesc = createTemplateDescByName(bcontext, KernelConst.RULE_TAGNAME, drawrule);
			...
			setRuleDesc(ruledesc, groupname);
...
//TODO			l_layers = getOrdredLayers(l_layers, level2type2props);
			for (IXMLObjectDesc l_layer : l_layers)
