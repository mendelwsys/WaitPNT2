 ...
 */
public class RegAppGenerator_axis extends BaseInitAble
...
//		trans.reinitMapTransformer(dstpar);
		...
		for (String typeid : type2record.keySet())
		{
			...
			for (RegReader.Record gobj : objs)
			{
				...
				final double[] ydim = gobj.getDimARRAY(RegReader.Record.LDIM);
				final double[] xdim = gobj.getDimARRAY(RegReader.Record.ADIM);


				cntpoints+=xdim.length;
...
//--------------------------------------- Инициализация конвертеров --------------------------------------------------//
		{
			...
			scaleBpoint = convGenerator.calcScale_BPoint2(mBB, rotconv, viewSz,false);
