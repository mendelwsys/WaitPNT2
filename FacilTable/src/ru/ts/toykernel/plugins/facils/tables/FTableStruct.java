...
import ru.ts.toykernel.plugins.facils.utils.TableStruct;
...
public class FTableStruct extends TableStruct implements IHeaderSupplyer
{
    ...
    public static final String ATTR_ORIG_LRID = KernelConst.ATTR_CURVE_CUSTOM + "_ORIG_LRID";
    public static final String ATTR_ORIG_OBJID =KernelConst.ATTR_CURVE_CUSTOM + "_ORIG_OBJID";
	public static final String ATTR_CNT_OBJ = KernelConst.ATTR_CURVE_CUSTOM + "_CNT_OBJ";
    public static final String ATTR_LR_GEN = KernelConst.ATTR_LAYER_CUSTOM + "_LR_GEN";
    ...
    /**
     * Отдать идентификатор объекта по коду дороги и кандидатам на оригинальный объект полученным по имени станции
     * @param dorcod - код дороги
     * @param stindex - кандидаты на оригинальный объект полученные по имени станции
     * @param layers - слои исходя из которых формируется идентификатор первичного объекта
     * @param ehandler - обработик ошибок
     * @return - <ID слоя,ID объекта> - идентификатор первичного объкта
     * @throws Exception  -
     */
//    static  public Pair<String, String> getStationOrig_Id(String dorcod, Map<String, List<String>> stindex,List<Layer> layers, CreateLayersErrorHandler ehandler) throws Exception
//    {
//        Pair<String,String> orig_id=null;//Оригинальный объект, если производится попытка установить второй раз выдается сообщение об ошибке
//        br:
//        if (stindex!=null)
//        for (String lrindex : stindex.keySet())
//        {
//            Layer lr=Layer.getLayerById(layers,lrindex);
//            List<String> stonlr=stindex.get(lrindex);
//            for (String stonlrindex : stonlr)
//            {
//                Curve curve=lr.incarnateByCurveId(stonlrindex);
//                if (curve!=null)
//                {
//                    String boundst= (String) curve.getRealAttr(AttrsConst.ATTR_CURVE_BOUND);
//                    String[] rv=getCodesByStation(boundst);
//                    if (rv!=null && rv[0].equals(dorcod))
//                    {//Если дороги совпадают устанавливаем код объекта и код оригинала
//                        if (orig_id==null)
//                            orig_id= new Pair<String,String>(lrindex,stonlrindex);
//                        else
//                        {
//                            Pair<String, Integer> pair = new Pair<String, Integer>(CreateLayersErrorHandler.ambiguity_err.first, CreateLayersErrorHandler.ambiguity_err.second);
//                            Pair<String, String> neworig_id = new Pair<String, String>(lrindex, stonlrindex);
//                            String boundst_old = (String) (Layer.getCurveById(orig_id, layers)).getRealAttr(AttrsConst.ATTR_CURVE_BOUND);
//                            pair.first+=Enc.get("WAS_")+orig_id.toString()+Enc.get("_LINK_ID_")+boundst_old +Enc.get("NOW_")+ neworig_id+Enc.get("_LINK_ID_")+boundst+Enc.get("NAME_")+(String) curve.getRealAttr(AttrsConst.ATTR_CURVE_NAME);
//                            int errcode = ehandler.onError(pair);
//                            if(errcode ==0)
//                                orig_id=neworig_id;
//                            else if (errcode==-1)
//                            {
...
//    public void createLayersByDocuments(List<Layer> layers,Map<String, Map<String, List<String>>> snames,NSIRoadsTable rtable, CreateLayersErrorHandler ehandler) throws Exception
//    {
//        Map<String, Layer> type2layer=new HashMap<String,Layer>();
//        for (Layer layer : layers) //Сформировать отображение по загруженными слоям
//        {
//            IAttrDefGroup lrgrp = Layer.getLayerIAttrDefGroup(layer);
//            IAttrDef attrDef = lrgrp.get_def(ATTR_LR_GEN);
//            if (attrDef!=null)
//                type2layer.put(attrDef.get_default_value().toString(),layer);
//        }
//
//        Map< Pair<String,String>,Map<String,String> > orig2ids=new HashMap<Pair<String,String>,Map<String,String>>();//отображение идентификатор оригинального объекта -> (идентификатор слоя->идентификатор объекта в слое)
//        for (Layer layer: type2layer.values())//Сформировать отображение по загруженными слоям
//        {
//            String[] curvIds=layer.getCurvesIds();
//            for (String curvId : curvIds)
//            {
//                Curve curve=layer.incarnateByCurveId(curvId);
//                String orig_lrid= (String) curve.getRealAttr(ATTR_ORIG_LRID);
//                String orig_objid= (String) curve.getRealAttr(ATTR_ORIG_OBJID);
//                if (orig_lrid!=null && orig_objid!=null)
//                {
//                    Pair<String, String> key = new Pair<String, String>(orig_lrid, orig_objid);
//                    Map<String,String> layer2curveid=orig2ids.get(key);
//                    if (layer2curveid==null)
//                    {
//                        layer2curveid = new HashMap<String,String>();
//                        orig2ids.put(key, layer2curveid);
//                    }
//                    layer2curveid.put(layer.getLayerId(),curvId);
//                }
//            }
//        }
//
//
//
//        //Распредилить объекты по слоям, если слой не найден создать и добавить его
//        List<String> rawactnames=getColumn(RAWFTable.FN4LN);
//        List<String> rawstname=getColumn(RAWFTable.STNM);
//        List<String> rawdorn=getColumn(RAWFTable.DORNM);
//		ILayerFactory ifactoryLayer = new FLayer();
//
//        for (int i = 0; i < rawactnames.size(); i++)
//        {
//            //Получить код дороги по ее названию из справочника
//            String dorcod=rtable.getRoadCodeByName(rawdorn.get(i));
//            if (dorcod==null || dorcod.length()==0)
//            {
//                ehandler.onError(CreateLayersErrorHandler.roadecode_err);
//                continue;
//            }
//
//            String actname = rawactnames.get(i);//Получить имя актива
//
//            //Создаем слой для активов этого типа
//            Layer lract = type2layer.get(actname);
//            if (lract == null)
//            {
//				lract = Layer.getEmptyLayer(new CommonStyle(0xFF00FF00, 0xFF00FF00), ifactoryLayer);
//                IAttrDefGroup lobjrgrp = lract.getObjectsIAttrDefGroup();
//                lobjrgrp.add_def(lobjrgrp.getNewAttrDef(ATTR_ORIG_LRID, IAttrDef.STRING, ""));
//                lobjrgrp.add_def(lobjrgrp.getNewAttrDef(ATTR_ORIG_OBJID, IAttrDef.STRING, ""));
//				lobjrgrp.add_def(lobjrgrp.getNewAttrDef(ATTR_CNT_OBJ, IAttrDef.STRING, "1"));
//
//                IAttrDefGroup lrgrp = Layer.getLayerIAttrDefGroup(lract);
//                lrgrp.add_def(lrgrp.getNewAttrDef(ATTR_LR_GEN, IAttrDef.STRING, actname));
//
//                lract.setName(actname);
//                lract.setVisible(true);
//                type2layer.put(actname, lract);
//                layers.add(lract);
//            }
//
//            //По имени и названию дороги получаем остановочный пункт
//            Pair<String, String> orig_id= getStationOrig_Id(dorcod, snames.get(rawstname.get(i)),layers,ehandler);
//            String lractid = lract.getLayerId();
//            if (orig_id!=null && orig2ids.get(orig_id)==null)
//                orig2ids.put(orig_id,new HashMap<String,String>());
//            else if (orig_id==null)
//            {
//                Pair<String, Integer> pair = CreateLayersErrorHandler.notorigpnt_err;
//                ehandler.onError(new Pair<String,Integer>(pair.first+" "+rawstname.get(i),pair.second));
//            }
//
//            if (orig_id!=null) //проверка что на этом слое еще нет объекта соотнесенного с таким первичным объектом
//            {
//
//                Curve orig_curve = Layer.getCurveById(orig_id, layers);
//                String boundst = (String) orig_curve.getRealAttr(AttrsConst.ATTR_CURVE_BOUND);
//
//
//				if (!orig2ids.get(orig_id).keySet().contains(lractid))
//                {
//                    //создаем объект на слое
//                    Structures.MyPoint[][] myPoints = orig_curve.getPoints();
//                    if (myPoints.length==1 && myPoints[0].length==1)//проверяем станция ли это
//                    {
//                        String curveId = lract.getnextObjId("NEWOBJ");//Создаем новый объект в выбранном слое
//                        HashMap<String, Object> attrs = new HashMap<String, Object>();
//
//                        attrs.put(AttrsConst.ATTR_CURVE_NAME, (String) orig_curve.getRealAttr(AttrsConst.ATTR_CURVE_NAME)+" "+actname);
//                        attrs.put(AttrsConst.ATTR_CURVE_BOUND, (String) orig_curve.getRealAttr(AttrsConst.ATTR_CURVE_BOUND));
//                        attrs.put(ATTR_ORIG_LRID, orig_id.first);
//                        attrs.put(ATTR_ORIG_OBJID, orig_id.second);
//
//                        Curve curve = new Curve(lract, new Structures.MyPoint[]{new Structures.MyPoint(myPoints[0][0].x,myPoints[0][0].y)}, curveId,
//                                AttrsConst.POINT, attrs);
//                        String newcurveid = lract.addCurve(curve);
//                        //заполняем таблицу дополнительных свойств
//                        setAddCols(i,orig_id,new Pair<String,String>(lractid,newcurveid),boundst);
//                        orig2ids.get(orig_id).put(lractid,newcurveid);
//                    }
//                    else
//                        ehandler.onError(CreateLayersErrorHandler.notpnt_err);
//                }
//                else
//				{
//					String actobj = orig2ids.get(orig_id).get(lractid);
//
//					Curve actcurve=Layer.getCurveById(new Pair<String,String>(lractid,actobj), layers);//Объект
//					IAttrs attrs=actcurve.getAttrs();
//					int objcnt= Integer.parseInt((String) attrs.get(ATTR_CNT_OBJ))+1;
//					attrs.set(ATTR_CNT_OBJ,String.valueOf(objcnt));
//					setAddCols(i,orig_id,new Pair<String,String>(lractid, actobj),boundst);
//				}
//			}
//
//        }
//    }
    ...
    static  public String[] getCodesByStation(String boundst)
    {
        ...
        if (boundst != null && boundst.indexOf(";") < 0)
        {
                ...
                rv[2] = boundst.substring(index1+1, boundst.length()).trim();
        }
        return rv;
    }
